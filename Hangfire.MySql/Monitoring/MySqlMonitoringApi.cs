...
select sum(s.`Value`) from (
    ...
    select `Value` from AggregatedCounter where `Key` = @key
        ...
        public JobList<SucceededJobDto> SucceededJobs(int @from, int count,string name = null)
        {
            return UseConnection(connection => GetJobs(
                connection,
                from,
        ...
        private JobList<TDto> GetJobs<TDto>(
            ...
            int count,
            string stateName,
            Func<SqlJob, Job, Dictionary<string, string>, TDto> selector,string name = null)
        {
            string jobsSql =
@"select * from (
...
@"select * from (
  ...
  from Job j
    cross join (SELECT @rownum := 0) r
  left join State s on j.StateId = s.Id
  where j.StateName = @stateName and s.Reason= @name
  order by j.Id desc
) as j where j.rank between @start and @end ";
            }
            ...
            var jobs =
               connection.Query<SqlJob>(
                   jobsSql,
                   new { stateName = stateName, start = @from + 1, end = @from + count,name = name })
                   .ToList();
